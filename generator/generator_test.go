...
func TestGeneratorBuildTemplateVars(t *testing.T) {
	parser := NewParser()
	parser.Parse(testFile)
	iface, _ := parser.Find("Requester")
	gen := NewGenerator(iface)

	vars, err := gen.BuildTemplateVars()
	assert.NoError(t, err)

	assert.Equal(t, vars, TemplateVars{
...
func (m *MockRequester) Get(path string) (string, error) {
	ret := m.traceCall(m.Get, path)
