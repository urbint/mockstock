...
func TestParseConfigDefaults(t *testing.T) {
	config := configFromCommandLine("mockstock")
...
func TestParseConfigFlippingValues(t *testing.T) {
	config := configFromCommandLine("mockstock -name hi -print -output output -dir dir -recursive -all -inpkg -testonly -case case -note note")
